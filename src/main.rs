...
use gran::{
    effects::{Pattern, PatternBeat, Volume}, oscillators::{Number, OscillatorBuilder, WaveFunction}, play_sound, sounds::{CompositionBuilder, SampleBuilder, Sound}
};
...
async fn main() {
    ...
    let sine_frequency = OscillatorBuilder::new()
        .wave_function(WaveFunction::Sine {
            ...
            amplitude: Number::number(10.0),  // +/- 5Hz
            phase: Number::number(0.0),
        })
        .beat_length(1.0)
        .build();
    let sine_frequency = Number::oscillator(sine_frequency).plus_f32(440.0);
